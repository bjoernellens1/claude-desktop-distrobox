...
- **A "Terminal entering Claude-desktop" entry opens and closes a terminal window instead of launching the app**: that's distrobox's own generic "enter this container" shortcut, not the Claude Desktop launcher — the two look similar in some app menus. There should only be one real entry now (just "Claude", or "Claude (on claude-desktop)"), since `entry=0` in the manifests suppresses the generic one. If you still see it, it's likely a stale leftover from before this fix — remove it with `rm ~/.local/share/applications/claude-desktop*.desktop` and re-run `./install.sh`.
